readSnpMatrix <- function(filename, skip=0L, err.thresh=Inf, del.thresh=Inf, perl.pileup=FALSE) {
    ...
    if (perl.pileup) {
        ...
        pileup <- readr::read_csv(filename, col_types=cols(
...
preProcSample <- function(rcmat, ndepth=35, het.thresh=0.25, snp.nbhd=250, cval=25, deltaCN=0, gbuild=c("hg19", "hg38", "hg18", "mm9", "mm10", "udef"), ugcpct=NULL, hetscale=TRUE, unmatched=FALSE, ndepthmax=1000) {
    ...
    if (gbuild == "udef") {
        tmp <- segsnps(counts2logROR(pmat, gbuild, unmatched, ugcpct), cval, hetscale, deltaCN)
    } else {
        tmp <- segsnps(counts2logROR(pmat, gbuild, unmatched), cval, hetscale, deltaCN)
    }
    gc(verbose=TRUE, reset=TRUE, full=TRUE)
    cat("END seqsnps\n")
